...
export default function TaxRefund() {
  ...
  const chips = [
    ...
    month && { k: "申报月", v: month, clear: () => set({ month: null }) },
    buyer && { k: "采购员", v: buyer, clear: () => set({ buyer: null }) },
    onlyUnlinked && { k: "只看", v: "未关联订单", clear: () => set({ unlinked: null }) },
  ...
  const columns: Column<TaxRow>[] = useMemo(
    () => [
      ...
      { key: "qty", title: "数量", width: 96, align: "right", sort: (a, b) => a.qty - b.qty, render: (r) => <span className="cell-num">{formatInt(r.qty)}</span> },
      { key: "gross", title: "价税合计", width: 118, align: "right", sort: (a, b) => a.gross - b.gross, render: (r) => <span className="cell-num">{formatMoney(r.gross, "¥")}</span> },
      { key: "net", title: "不含税金额", width: 118, align: "right", sort: (a, b) => a.net - b.net, render: (r) => <span className="cell-num muted">{formatMoney(r.net, "¥")}</span> },
      {
        key: "tax",
        title: "税额",
        width: 112,
      ...
      { key: "customs", title: "报关单号", width: 158, render: (r) => <span className="num truncate" style={{ display: "block", fontSize: "var(--fs-sm)" }}>{r.customsNo ?? "—"}</span> },
      { key: "usd", title: "报关美元", width: 112, align: "right", sort: (a, b) => a.customsUsd - b.customsUsd, render: (r) => <span className="cell-num">{r.customsUsd ? formatMoney(r.customsUsd) : "—"}</span> },
      { key: "exported", title: "出口日期", width: 106, sort: (a, b) => (a.exportedOn ?? "").localeCompare(b.exportedOn ?? ""), render: (r) => <span className="num">{r.exportedOn ?? "—"}</span> },
      { key: "buyer", title: "采购员", width: 82, render: (r) => r.buyer },
      { key: "entity", title: "开票主体", width: 100, render: (r) => <Pill tone={r.entity === "供应链" ? "violet" : "accent"}>{r.entity ?? "—"}</Pill> },
    ],
    [readOnly],
  );
  ...
  const doExport = async () => {
    await exportXlsx<TaxRow>(
      stampName("退税明细"),
      [
        { header: "申报月", width: 10, value: (r) => r.declareMonth },
        { header: "批次", width: 8, value: (r) => r.batch },
        { header: "发票号", width: 14, value: (r) => r.invoiceNo },
        { header: "关联 PI", width: 16, value: (r) => r.piNo },
        { header: "销售方", width: 32, value: (r) => r.sellerName },
        { header: "商品名称", width: 32, value: (r) => r.item },
        { header: "数量", width: 12, type: "number", format: "#,##0", value: (r) => r.qty },
        { header: "价税合计", width: 14, type: "number", value: (r) => r.gross },
        { header: "不含税", width: 14, type: "number", value: (r) => r.net },
        { header: "税额", width: 14, type: "number", value: (r) => r.tax },
        { header: "出口日期", width: 12, type: "date", value: (r) => r.exportedOn },
        { header: "报关单号", width: 20, value: (r) => r.customsNo },
        { header: "报关美元", width: 14, type: "number", value: (r) => r.customsUsd },
        { header: "采购员", width: 10, value: (r) => r.buyer },
        { header: "开票主体", width: 12, value: (r) => r.entity },
      ],
      rows,
    );
  ...
  return (
    <div className="page">
      <div className="page-head">
        <div>
          <h1>退税管理</h1>
          <p>出口退税发票明细台账 · 未关联订单的行会标红并可一键挂到 PI</p>
        </div>
      ...
      <div className="kpis">
        ...
        <div className="kpi">
          ...
          <span className="kpi-s">切换申报月只影响这张卡</span>
        </div>
        <div className="kpi">
          <span className="kpi-k">
            <Icon name="inbox" />
          ...
          <span className="kpi-s">当前主体全量</span>
        </div>
        <button className="kpi" data-tone={kpi.unlinked ? "coral" : "jade"} onClick={() => set({ unlinked: onlyUnlinked ? null : "1" })}>
          ...
          <span className="kpi-s">{kpi.unlinked ? "点这里只看它们" : "全部已挂到 PI"}</span>
        </button>
      </div>

      <div className="toolbar">
        <SearchInput value={q} onChange={(v) => set({ q: v })} placeholder="搜发票号 / 报关单号 / 销售方 / 商品…" />
        <Segmented
          value={entity}
          onChange={(v) => set({ entity: v })}
          options={[{ value: "", label: "全部主体" }, ...entities.map((e) => ({ value: e, label: e }))]}
          label="开票主体"
        />
        <span className="toolbar-sep" />
        <select className="select" value={month} onChange={(e) => set({ month: e.target.value })} aria-label="申报月">
        ...
        <select className="select" value={buyer} onChange={(e) => set({ buyer: e.target.value })} aria-label="采购员">
      ...
      <DataGrid<TaxRow>
        ...
        bar={
          <>
            <span>
              本页 <b className="num">{rows.length}</b> 行
            </span>
        ...
        empty={<EmptyState icon="file" title="当前筛选下没有发票" desc="换个申报月或开票主体试试。" />}
        renderCard={(r) => (
      ...
      {linking ? (
        <LinkWizard
          ...
          onLink={(piId, piNo) => {
            ...
            setLinking(null);
            toast(`${linking.invoiceNo} 已挂到 ${piNo}`, () => {
  ...
  return (
    ...
    >
      <div style={{ display: "grid", gap: 12, paddingBottom: 6 }}>
        ...
        <SearchInput value={q} onChange={setQ} placeholder="搜 PI 号 / 客户 / 产品…" autoFocus />

        <div style={{ maxHeight: 320, overflowY: "auto", display: "grid", gap: 2 }}>
          {candidates.length === 0 ? (
            <EmptyState icon="search" title="没有匹配的 PI" desc="换个关键词，或者先去 PI 取号建一张。" />
          ) : (
            candidates.map((c) => (
              <button
                key={c.id}
              ...
              >
                <div className="citem-top">
                  ...
                  {c.score > 0.9 ? <Pill tone="jade">金额高度吻合</Pill> : c.score > 0.7 ? <Pill tone="accent">金额接近</Pill> : null}
                  <span className="spacer" />
